...
/**
  ...
  ChipInfo and PlatformInfo libraries or SMEM.
  ...
  )
{
  ...
  ChipInfoVersion                  = ChipInfo_GetChipVersion ();
  ChipPlatInfoProp->ChipFamily     = (UINT16)ChipInfo_GetChipFamily ();
  ChipPlatInfoProp->ChipId         = (UINT16)ChipInfo_GetChipId ();
  ...
  pPlSem = (PlatformInfoSMemType *)SmemGetAddr (SmemHwSwBuildId, &BufferSize);

  if (pPlSem == NULL) {
    BoardInfoVersion                     = PlatformInfo_PlatformVersion ();
    ChipPlatInfoProp->PlatformType       = (UINT8)PlatformInfo_Platform ();
    ChipPlatInfoProp->PlatformSubtype    = (UINT8)PlatformInfo_PlatformSubtype ();
    ...
    ChipPlatInfoProp->OemVar             = PlatformInfo_OEMVariant ();
  } else {
    BoardInfoVersion                     = pPlSem->nPlatformVersion;
    ChipPlatInfoProp->PlatformType       = (UINT8)pPlSem->ePlatformType;
    ChipPlatInfoProp->PlatformSubtype    = (UINT8)pPlSem->nPlatformSubtype;
    ChipPlatInfoProp->PlatformMajVersion = (UINT8)(BoardInfoVersion >> (sizeof (UINT16) * 8));
    ChipPlatInfoProp->PlatformMinVersion = (UINT8)(BoardInfoVersion);
    ChipPlatInfoProp->OemVar             = pPlSem->nOEMVariantID;
  }
