/** @file
  ...
  UEFI Device Tree Extension Protocol
...
/// Protocol GUID for EFI DTB Extension Protocol.
...
#define EFI_DTB_EXTN_PROTOCOL_VER_REV2  0x00010002
/// Revision 3 of the protocol with additional DT APIs for boolean property support.
#define EFI_DTB_EXTN_PROTOCOL_VER_REV3  0x00010003

/// Current protocol version.
#define EFI_DTB_EXTN_PROTOCOL_VERSION  EFI_DTB_EXTN_PROTOCOL_VER_REV3

/// Forward declaration of FDT node handle type.
...
/**
  ...
  @param[in]      BlobId  Index of the blob to retrieve (see DEFAULT_BLOB_ID/MAX_BLOB_ID).

  @retval FDT_ERR_QC_NOERROR  The blob handle was retrieved successfully.
  @retval Others              A negative FDT_ERR_QC_ (or underlying libfdt) error code indicating the failure reason.
**/
typedef INT32 (*FDT_GET_BLOB_HANDLE)(
...
/**
  ...
  @retval FDT_ERR_QC_NOERROR  The blob size was retrieved successfully.
  @retval Others              A negative FDT_ERR_QC_ (or underlying libfdt) error code indicating the failure reason.
**/
typedef INT32 (*FDT_GET_BLOB_SIZE)(
...
/**
  ...
  @retval FDT_ERR_QC_NOERROR  The root handle was initialized successfully.
  @retval Others              A negative FDT_ERR_QC_ (or underlying libfdt) error code indicating the failure reason.
**/
typedef INT32 (*FDT_INIT_ROOT_HANDLE_FOR_DRIVER_BY_BLOB)(
  IN OUT FDT_NODE_HANDLE  *Node,
  IN CONST VOID           *Blob
  );
...
/**
  ...
  Uses the default registered blob (DEFAULT_BLOB_ID) as the backing device tree.
...
/**
  ...
  @retval Others              A negative FDT_ERR_QC_ (or underlying libfdt) error code indicating the failure reason.
**/
typedef INT32 (*FDT_INIT_ROOT_HANDLE_FOR_DRIVER_BY_ID)(
  IN OUT FDT_NODE_HANDLE  *Node,
  IN INT32                BlobId
  );
...
/**
  ...
  @retval FDT_ERR_QC_NOERROR  The blob handle was registered successfully.
  @retval Others              A negative FDT_ERR_QC_ (or underlying libfdt) error code indicating the failure reason.
**/
typedef INT32 (*FDT_SET_BLOB_HANDLE)(
  IN CONST VOID  *Blob,
  IN UINTN       Size,
  IN INT32       BlobId
  );
...
/**
  ...
  @retval FDT_ERR_QC_NOERROR  Blob is a valid device tree blob of at least Size bytes.
  @retval Others              A negative FDT_ERR_QC_ (or underlying libfdt) error code indicating the failure reason.
**/
typedef INT32 (*FDT_CHECK_VALID_BLOB)(
  IN CONST VOID  *Blob,
  IN UINTN       Size
  );
...
/**
  ...
  @retval FDT_ERR_QC_NOERROR  TName was found and Index was updated.
  @retval Others              A negative FDT_ERR_QC_ (or underlying libfdt) error code indicating the failure reason
...
/**
  ...
  @retval FDT_ERR_QC_NOERROR  A matching node was found and Node was updated.
  @retval Others              A negative FDT_ERR_QC_ (or underlying libfdt) error code indicating the failure reason.
**/
...
/**
  ...
  @param[in]      AddrCode   Address field width in cells: SIZE_32 or SIZE_64 (SIZE_BLOB/"from tree" is not currently supported).
  @param[in]      SizeCode   Size field width in cells: SIZE_NIL (no size field), SIZE_32, or SIZE_64
                              (SIZE_BLOB/"from tree" is not currently supported).
  @param[in, out] RegPaddr   On output, physical address decoded from the selected reg entry.
  @param[in, out] RegSize    On output, size decoded from the selected reg entry, or 0 if SizeCode is SIZE_NIL.

  @retval FDT_ERR_QC_NOERROR  The reg entry was decoded successfully.
  @retval Others              A negative FDT_ERR_QC_ (or underlying libfdt) error code indicating the failure reason.
**/
...
/**
  ...
  @retval FDT_ERR_QC_NOERROR  Count was updated successfully.
  @retval Others              A negative FDT_ERR_QC_ (or underlying libfdt) error code indicating the failure reason.
**/
...
/**
  ...
  @retval FDT_ERR_QC_NOERROR  Node was found and the handle was initialized.
  @retval Others              A negative FDT_ERR_QC_ (or underlying libfdt) error code indicating the failure reason.
**/
typedef INT32 (*FDT_GET_NODE_HANDLE_BY_BLOB)(
  IN OUT FDT_NODE_HANDLE  *Node,
  IN CONST VOID           *Blob,
  IN CHAR8                *Name
  );
...
/**
  ...
  Equivalent to FdtGetNodeHandleByBlob using the default registered blob (DEFAULT_BLOB_ID).
...
/**
  ...
  @retval FDT_ERR_QC_NOERROR  Pnode was initialized successfully.
  @retval Others              A negative FDT_ERR_QC_ (or underlying libfdt) error code indicating the failure reason.
**/
...
/**
  ...
  @retval FDT_ERR_QC_NOERROR  PropNames was populated successfully.
  @retval Others              A negative FDT_ERR_QC_ (or underlying libfdt) error code indicating the failure reason.
**/
...
/**
  ...
  @retval FDT_ERR_QC_NOERROR  Size was updated successfully.
  @retval Others              A negative FDT_ERR_QC_ (or underlying libfdt) error code indicating the failure reason.
**/
...
/**
  ...
  @retval 0                            Success.
  @retval -FDT_ERR_QC_NULLPTR          A required pointer argument was NULL.
  @retval -FDT_ERR_QC_NILVALUE         A property value's size is 0.
  @retval -FDT_ERR_QC_SDNULL           Failed to get the address of a property value in the blob.
  @retval -FDT_ERR_QC_TRUNCATED        Size is smaller than the total size of the node's properties.
  @retval -FDT_ERR_QC_BADFORMAT        Format contains a letter outside the documented set.
...
/**
  ...
  @retval FDT_ERR_QC_NOERROR  NameSizes was populated successfully.
  @retval Others              A negative FDT_ERR_QC_ (or underlying libfdt) error code indicating the failure reason.
**/
...
/**
  ...
  @retval FDT_ERR_QC_NOERROR  Names was populated successfully.
  @retval Others              A negative FDT_ERR_QC_ (or underlying libfdt) error code indicating the failure reason.
**/
...
/**
  ...
  @retval -FDT_ERR_QC_NODE_DIFFERENT   NodeA and NodeB refer to different nodes.
  @retval Others                       A negative FDT_ERR_QC_ (or underlying libfdt) error code indicating the failure reason
                                        (e.g. a NULL node pointer).
**/
...
/**
  ...
  @retval FDT_ERR_QC_NOERROR  Dst was populated successfully.
  @retval Others              A negative FDT_ERR_QC_ (or underlying libfdt) error code indicating the failure reason.
**/
...
/**
  ...
  @retval FDT_ERR_QC_NOERROR  The call completed successfully.
  @retval Others              A negative FDT_ERR_QC_ (or underlying libfdt) error code indicating the failure reason.
**/
...
/**
  ...
  @retval FDT_ERR_QC_NOERROR  PropName is present on Node.
  @retval Others              A negative FDT_ERR_QC_ (or underlying libfdt) error code indicating the failure reason
                              (e.g. the property is not present).
**/
typedef INT32 (*FDT_GET_BOOL_PROP)(
  IN FDT_NODE_HANDLE  *Node,
  IN CONST CHAR8      *PropName
  );
...
/**
  ...
  @retval FDT_ERR_QC_NOERROR  StringList was populated successfully.
  @retval Others              A negative FDT_ERR_QC_ (or underlying libfdt) error code indicating the failure reason.
**/
...
/**
  ...
  @retval FDT_ERR_QC_NOERROR  PropList was populated successfully.
  @retval Others              A negative FDT_ERR_QC_ (or underlying libfdt) error code indicating the failure reason.
**/
...
/**
  ...
  @retval FDT_ERR_QC_NOERROR  Cache was populated successfully.
  @retval Others              A negative FDT_ERR_QC_ (or underlying libfdt) error code indicating the failure reason.
**/
...
/**
  ...
  @retval FDT_ERR_QC_NOERROR   The overlay was merged successfully.
  @retval -FDT_ERR_QC_BUF2SMALL  MergeBlob is not large enough to hold the merged blob.
  @retval Others               A negative FDT_ERR_QC_ (or underlying libfdt) error code indicating the failure reason.
**/
typedef INT32 (*FDT_MERGE_OVERLAY)(
...
/**
  EFI Device Tree Extension Protocol structure.
**/
typedef struct {
  UINT64                                     Version;
  ...
  FDT_SET_BLOB_HANDLE                        FdtSetBlobHandle;
  FDT_CHECK_VALID_BLOB                       FdtCheckValidBlob;
  FDT_MERGE_OVERLAY                          FdtMergeOverlay;
  FDT_GET_BLOB_HANDLE                        FdtGetBlobHandle;
  FDT_GET_BLOB_SIZE                          FdtGetBlobSize;
  FDT_INIT_ROOT_HANDLE_FOR_DRIVER_BY_ID      FdtInitRootHandleForDriverById;
  FDT_GET_NODE_HANDLE_BY_BLOB                FdtGetNodeHandleByBlob;
  FDT_INIT_ROOT_HANDLE_FOR_DRIVER_BY_BLOB    FdtInitRootHandleForDriverByBlob;
  FDT_GET_BOOL_PROP                          FdtGetBoolProp;
